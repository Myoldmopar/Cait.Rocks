...
class TestDirectionAPIMethods(TestCase):
    def test_get_empty_directions(self):
        url_path = reverse('planner:api:direction-list')
        self.assertEqual('/planner/api/directions/', url_path)
    ...
    def test_get_detail_item_invalid(self):
        url_path = reverse('planner:api:direction-detail', args=[1])
        response = self.client.get(url_path)
    ...
    def test_get_detail_item_valid(self):
        ...
        url_path = reverse('planner:api:direction-detail', args=[1])
        self.assertEqual('/planner/api/directions/1/', url_path)
        response = self.client.get(url_path)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        body = json.loads(response.content)
