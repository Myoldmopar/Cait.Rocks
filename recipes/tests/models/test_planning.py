...
class CalendarDayModelConstructionTests(TestCase):

    def test_calendar_day_default_construction(self):
        """
        Makes sure calendar day can potentially be empty...do we want this?
        """
        blank_calendar_day = CalendarDay()
        self.assertEqual(str(blank_calendar_day), u'A single calendar day instance, containing up to 4 recipes')

    def test_calendar_day_with_nickname_only(self):
        title_string = 'This is a normal calendar'
        titled_calendar = CalendarDay(nickname=title_string)
        self.assertEqual(str(titled_calendar), u'A single calendar day instance, containing up to 4 recipes')
