...
class TestRecipeAPIMethods(TestCase):
    def test_get_empty_directions(self):
        url_path = reverse('api:recipe-list')  # TODO: Why does recipes-list show up with list_url_names but not resolve
    ...
    def test_get_populated_directions(self):
        ...
        url_path = reverse('api:recipe-list')
        response = self.client.get(url_path)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        body = json.loads(response.content)
        self.assertIsInstance(body, list)
    ...
    def test_get_detail_item_invalid(self):
        url_path = reverse('api:recipe-detail', args=[1])
        response = self.client.get(url_path)
